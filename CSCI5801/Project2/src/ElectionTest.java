...
//				new File("../testing/testWriteFile3.csv")
				new File("../testing/testInvalid.csv"),
				new File("../testing/testInvalid1.csv"),
				new File("../testing/testInvalid2.csv")
		};

		File[] plurFiles = {new File("../testing/testPlurality.csv")};
		Election election = new Election("STV", 2, files, true);
		Election electionPlur = new Election("Plurality", 2, plurFiles, false);
		election.setTesting("../testing/testOutputs/testCreateBallotListSTV_output.txt");
		electionPlur.setTesting("../testing/testOutputs/testCreateBallotListPlur_output.txt");

		// Call to create the list of ballots
		election.createBallotList();
		electionPlur.createBallotList();

		// get the ballots and open the file that ballots should have been
		// written to
		LinkedHashSet<Ballot> ballots = election.getBallotList();
		LinkedHashSet<Ballot> plurBallots = electionPlur.getBallotList();

		BufferedReader fileReader = new BufferedReader(new FileReader("../testing/testOutputs/testCreateBallotListSTV_output.txt"));
		BufferedReader fileReader1 = new BufferedReader(new FileReader("../testing/testOutputs/testCreateBallotListPlur_output.txt"));
		...
		assertEquals(electionPlur.getNumBallots(), plurBallots.size());
	}
	...
	/**
	 * This test checks that invalidated ballots are caught during initial
	 * processing and are written to the separate invalidate ballots report.
	 * @throws IOException
	 * @result invalidated ballots are correctly caught in the input files and
	 * 			written to Invalidated_Ballots.txt.
	 */
	@ParameterizedTest
	@ValueSource(strings = { "../testing/testInvalid.csv", "../testing/testInvalid1.csv",
			"../testing/testInvalid2.csv", "../testing/testInvalid3.csv",
			"../testing/testInvalid4.csv", "../testing/testInvalid5.csv",
			"../testing/testInvalid6.csv"
	})
	public void testWriteInvalidatedBallots(String testFile) throws IOException {
		File[] files = { new File (testFile) };
		Election election = new Election("STV", 2, files, true);
		
		// keep track of invalid ballots
		ArrayList<String> invalid = new ArrayList<String>();
		
		invalid.add("-------------------------------------------------------");
		invalid.add("-------------    Invalidated Ballots    ---------------");
		invalid.add("-------------------------------------------------------");
		invalid.add("");
		
		// loop through files, add incorrect lines to invalid
		BufferedReader fileReader = new BufferedReader(new FileReader(testFile));
		election.setCandidates(fileReader.readLine());
		
		// loop over ballots, add invalid ones to invalid list
		int count = 0, lineNumber = 2;
		String line;
		while ((line = fileReader.readLine()) != null) {
			if (!election.isValid(line)) {
				count++;
				StringBuilder output = new StringBuilder();
				output.append("Invalid ballot number " + count);
				output.append(": " + line);
				output.append(" | Line number " + lineNumber);
				output.append(" of file: " + files[0].getName());
				invalid.add(output.toString());
			}
			lineNumber++;
		}
		...
		// call election.createBallotList()
		election = new Election("STV", 2, files, true);
		election.createBallotList();
		
		// read through "../Invalidated_Ballots.txt" and assert each line
		fileReader = new BufferedReader(new FileReader("../Invalidated_Ballots.txt"));
		line = null;
		int index = 0;
		while ((line = fileReader.readLine()) != null) {
			String expected = invalid.get(index);
			assertEquals(expected, line);
			index++;
		}
	}
	 ...
	 *
	 * This method is used to determine if a STV ballot is valid or not with an even amount of candidates.
	 *
	 * @param testValue - test string of STV ballots with an even number of candidates, comma delimited.
	 * @throws IOException
	 * @result the number of valid STV ballots. Given this specific test set there should
	 * 			be 4 valid ballots of the 6 params.
	 * */
	@ParameterizedTest
	@ValueSource(strings = { "1,2,3,4", "1,,,", ",,1,2", ",1,2,3", ",2,1,", ",,,1"})
	public void testIsValidEven(String testValue) throws IOException {
		Election election = generateTestElection("STV");
		election.numCandidates = 4;

		assertTrue(election.isValid(testValue));
	}

	/**
	 * Tests the isValid method.
	 *
	 * This method is used to determine if a STV ballot is valid or not with an odd amount of candidates.
	 *
	 * @param testValue - test string of STV ballots with an odd number of candidates, comma delimited.
	 * @throws IOException
	 * @result the number of valid STV ballots. Given this specific test set there should
	 * 			be 2 valid ballots of the 4 params.
	 * */
	@ParameterizedTest
	@ValueSource(strings = {"1,2,,,", "1,2,3,,", "1,2,3,4,5", ",,,,1"})
	public void testIsValidOdd(String testValue) throws IOException {
		Election election = generateTestElection("STV");
		election.numCandidates = 5;
