...
public class UI {
    ...
    private static File[] files = null; // files of ballots
    ...
    private static String helpMessage = " - Type 'h' and press enter for the help menu\n"
            + " - Type 'x' and press enter to quit\n"; // Message that gets called in every prompt
     ...
     * 
     * @param input - used during testing, the command to be passed in.
     */
    private static void promptFiles(String input) {
        System.out.println("");
        String message = "Please enter 'f' if you would like to pull up the GUI to select which files you want to be include in the election\n"
                + helpMessage;
        ...
        do {
            ...
            if (sysInput.equals("h")) { // Help
                ...
                invalid = false;
                return;
            } else if (sysInput.equals("f")) { // Open file GUI menu.
                if(testing){
                	invalid = false;
                    quitProgram = invalid;
                } else {
                	files = GUI.runFileGUI(); // Sets the global variable of files to the contents of filesArr
                	if (files != null) {
                		invalid = false;
                	}
                }
            } else { // Did not enter correct prompt.
                invalid = true;
            }
        } while (invalid);
    }
    ...
    private static void promptNumSeats(String input) {
    	System.out.println("");
        String message = "Please enter the number of seats: \n" + helpMessage;
    ...
    private static boolean confirmInput(String input) throws IOException {
        ...
        if (files == null) {
        	fileNames = "No files have been selected.";
        } else {
	        for (File file : files) {
        ...
        String message = " - Algorithm type: " + voteType + "\n"
    ...
    private static boolean displayResults() {
        ...
        try {
            ...
            System.out.println("\n---------------------------------\n------------ RESULTS ------------\n---------------------------------");
            String message = "Election Type: " + election.getVoteType() + "\n"
                    + "Number of Ballots: " + election.getNumBallots() + "\n" + "Number of Seats: "
                    + election.getNumSeats() + "\n" + "Number of Candidates: " + election.getNumCandidates() + "\n"
                    + "Winner(s): " + winners + "\n" + "Loser(s): " + losers + "\n"
                    + "See \"ElectionResults_AuditFile.txt\" in the Project directory "
