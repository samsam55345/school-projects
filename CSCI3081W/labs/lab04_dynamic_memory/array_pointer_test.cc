...
void ArrayPointerTest::NameTheDucks(int array_number) {
  ...
  switch(array_number) {
    ...
    case 3:
      ...
      //      for (int i=0; i< std::max(count_, kMaxDuckCount); i++) {

      for (int i=0; i<count_; i++) {
        ducks3_[i]->set_name(names[i]);
        ducks3_[i]->set_number(3);
        cout<< ducks3_[i]->get_name() << endl;
